...
namespace Sistema_Archivos
{
    public class NotificadorDAL
    {
        ...
        public DataTable NotificadorCruzBlanca(string fun)
        {
            SqlCommand sel = conectar.Comando();
            sel.CommandText = @"SELECT n.sa_notificador, n.sa_notificador_nombre
                FROM sa_archivo_cruz_blanca as ac right join sa_notificador as n on ac.sa_notificador = n.sa_notificador
                WHERE (ac.sa_notificador = n.sa_notificador) and (ac.sa_archivo_cruz_blanca_fun = @fun)
                UNION ALL
                SELECT n1.sa_notificador, n1.sa_notificador_nombre FROM sa_archivo_cruz_blanca as ac1, sa_notificador as n1
                WHERE (ac1.sa_notificador <> n1.sa_notificador) and (ac1.sa_archivo_cruz_blanca_fun = @fun)";
            sel.Parameters.AddWithValue("@fun", fun);
            SqlDataAdapter adaptador = new SqlDataAdapter(sel);
            DataTable tabla = new DataTable();
            adaptador.Fill(tabla);
