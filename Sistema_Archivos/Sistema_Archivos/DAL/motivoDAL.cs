...
namespace Sistema_Archivos
{
    public class MotivoDAL
    {
        ...
        public DataTable CargarListaSeleccionadoCruzBlanca(string fun)
        {
            SqlCommand sel = conectar.Comando();
            sel.CommandText = @"SELECT m.sa_motivo, m.sa_motivo_desc
                FROM sa_motivo as m right join sa_archivo_cruz_blanca as ac on m.sa_motivo = ac.sa_motivo
                WHERE (ac.sa_motivo = m.sa_motivo) and (ac.sa_archivo_cruz_blanca_fun = @fun)
                UNION ALL
                SELECT m1.sa_motivo, m1.sa_motivo_desc FROM sa_motivo as m1, sa_archivo_cruz_blanca as ac1
                WHERE (ac1.sa_motivo <> m1.sa_motivo) and (ac1.sa_archivo_cruz_blanca_fun = @fun)";
            sel.Parameters.AddWithValue("@fun", fun);
            SqlDataAdapter adaptador = new SqlDataAdapter(sel);
            DataTable tabla = new DataTable();
            adaptador.Fill(tabla);
