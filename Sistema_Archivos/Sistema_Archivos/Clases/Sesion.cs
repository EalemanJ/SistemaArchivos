...
namespace Sistema_Archivos
{
    public static class Sesion
    {
        ...
        public static bool VerificarInubicableEsencial(string rut)
        {
            try
            {
                System.Data.DataTable consulta = new ArchivoEsencial().VerInubicable(rut);
                if (consulta.Rows.Count > 0)
                {
                    //ESTA INUBICABLE
                    return true;
        ...
        public static bool VerificarInubicableCruzBlanca(string rut)
        {
            try
            {
                System.Data.DataTable consulta = new ArchivoCruzBlanca().VerInubicable(rut);
                if (consulta.Rows.Count > 0)
                {
                    //ESTA INUBICABLE
                    return true;
