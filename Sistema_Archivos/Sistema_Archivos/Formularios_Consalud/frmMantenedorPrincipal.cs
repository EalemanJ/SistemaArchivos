...
{
    public partial class frmMantenedorPrincipal : Form
    {
        public static bool estado_carga = false;
        public static int ubicacionDH = 0, ubicacionDFP = 0, ubicacionR = 0;

        public frmMantenedorPrincipal()
        {
            InitializeComponent();
            cmsOpcionesEliminar.Renderer = new MyRenderer();
            cmsOpcionesModificar.Renderer = new MyRenderer();
        }
        ...
        private void BtnCrearNotificador_Click(object sender, EventArgs e)
        {
            try
            {
                if (string.IsNullOrWhiteSpace(txtNombre.Text))
                {
                    MessageBox.Show("Ingrese el nombre del nuevo notificador.", "CAMPO VACÍO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtNombre.Focus();
                }
                else
                {
                    System.Data.DataTable consulta = new NotificadorDAL().BuscarPorNombre(txtNombre.Text);
                    if (consulta.Rows.Count >= 1)
                    {
                        MessageBox.Show("Ya existe un notificador con ese nombre.", "YA EXISTE", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        txtNombre.Text = "";
                    }
                    else
                    {
                        int ingreso = new NotificadorDAL().IngresoNotificador(txtNombre.Text);
                        MessageBox.Show("Notificador ingresado con éxito.", "INGRESO CORRECTO", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                        Limpiar_todo();
                    }
                }
            }
            ...
            {
                MessageBox.Show("Error al crear al notificador.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        ...
        public void Limpiar_todo()
        {
            ...
            txtFUN.Text = "";
            txtNombre.Text = "";
            txtNombreConsultar.Text = "";
            txtNombreEmpresa.Text = "";
            txtRutAfiliado.Text = "";
            txtTipoFun.Text = "";
            dgvNotificadores.DataSource = null;
            dgvNotificadorEliminar.DataSource = null;
            ...
            //modificiardgv
            dgvNotificadoresModificar.DataSource = null;
        ...
        private void BtnBuscar_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvNotificadores.DataSource = null;
                if (string.IsNullOrWhiteSpace(txtNombreConsultar.Text))
                {
                    MessageBox.Show("Ingrese el nombre del notificador.", "CAMPO VACÍO", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    txtNombre.Focus();
                }
                else
                {
                    System.Data.DataTable consulta = new NotificadorDAL().BuscarPorNombre(txtNombreConsultar.Text);
                    dgvNotificadores.DataSource = consulta;
                    Formato_dgv_notificadores();
                }
            }
            catch (Exception ex)
            {
        ...
        public void Formato_dgv_notificadores()
        {
            dgvNotificadores.RowsDefaultCellStyle.BackColor = Color.White;
            dgvNotificadores.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(204, 201, 201);
            dgvNotificadores.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(95, 135, 14);
            dgvNotificadores.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            foreach (DataGridViewColumn Col in dgvNotificadores.Columns)
            {
                Col.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
        }

        public void Formato_dgv_notificadores_eliminar()
        {
            dgvNotificadorEliminar.RowsDefaultCellStyle.BackColor = Color.White;
            dgvNotificadorEliminar.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(204, 201, 201);
            dgvNotificadorEliminar.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(95, 135, 14);
            dgvNotificadorEliminar.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            foreach (DataGridViewColumn Col in dgvNotificadorEliminar.Columns)
            {
                Col.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
        }
        ...
        private void BtnVerTodos_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvNotificadores.DataSource = null;
                System.Data.DataTable consulta = new NotificadorDAL().VerTodosNotificadores();
                dgvNotificadores.DataSource = consulta;
                Formato_dgv_notificadores();
            }
            catch (Exception ex)
        ...
        private void BtnCargarDatos_Click(object sender, EventArgs e)
        {
            try
            {
                dgvNotificadorEliminar.DataSource = null;
                System.Data.DataTable consulta = new NotificadorDAL().VerTodosNotificadores();
                dgvNotificadorEliminar.DataSource = consulta;
                Formato_dgv_notificadores_eliminar();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al buscar. Detalles: " + ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void TsmiEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvNotificadorEliminar.Rows.Count == 0)
                {
                    MessageBox.Show("No hay datos en el visualizador.", "SIN DATOS", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else
                {
                    if (dgvNotificadorEliminar.CurrentRow.Cells["Nombre Notificador"].Value.ToString() == "Sin Notificador")
                    {
                        MessageBox.Show("No se puede eliminar el Notificador por Defecto.", "ACCIÓN INCORRECTA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                    else
                    {
                        DialogResult mensaje = MessageBox.Show("¿Está seguro de eliminar el notificador seleccionado?.", "CONFIRMAR ELIMINACIÓN", MessageBoxButtons.YesNo, MessageBoxIcon.Asterisk);
                        if (mensaje == DialogResult.Yes)
                        {
                            int codigo = Convert.ToInt32(dgvNotificadorEliminar.CurrentRow.Cells["Código Notificador"].Value.ToString());
                            int nuevo_notificador = 1; //notificador por defecto
                            int actualizar_archivos = new ArchivoDAL().ActualizarNotificadorArchivo(nuevo_notificador, codigo);
                            int eliminar = new NotificadorDAL().EliminarNotificador(codigo);
                            if (eliminar >= 1)
                            {
                                MessageBox.Show("Notiicador eliminado con éxito.", "ELIMINACIÓN CORRECTA", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                                Limpiar_todo();
                            }
                        }
                    }
            ...
            {
                MessageBox.Show("Error al eliminar. Detalles: " + ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }

        public void Cargar_datos_dgv_modificar()
        {
            dgvNotificadoresModificar.DataSource = null;
            System.Data.DataTable consulta = new NotificadorDAL().VerTodosNotificadores();
            dgvNotificadoresModificar.DataSource = consulta;
            dgvNotificadoresModificar.RowsDefaultCellStyle.BackColor = Color.White;
            dgvNotificadoresModificar.AlternatingRowsDefaultCellStyle.BackColor = Color.FromArgb(204, 201, 201);
            dgvNotificadoresModificar.ColumnHeadersDefaultCellStyle.BackColor = Color.FromArgb(95, 135, 14);
            dgvNotificadoresModificar.ColumnHeadersDefaultCellStyle.ForeColor = Color.White;
            foreach (DataGridViewColumn Col in dgvNotificadoresModificar.Columns)
            {
                Col.SortMode = DataGridViewColumnSortMode.NotSortable;
            }
        }

        private void BtnCargarTodoModificar_Click(object sender, EventArgs e)
        {
            try
            {
                Cargar_datos_dgv_modificar();
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error al buscar. Detalles: " + ex.Message, "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void TsmiModificar_Click(object sender, EventArgs e)
        {
            try
            {
                if (dgvNotificadoresModificar.Rows.Count == 0)
                {
                    MessageBox.Show("No hay datos en el visualizador.", "SIN DATOS", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                }
                else
                {
                    if (dgvNotificadoresModificar.CurrentRow.Cells["Nombre Notificador"].Value.ToString() == "Sin Notificador")
                    {
                        MessageBox.Show("No se puede modificar el Notificador por Defecto.", "ACCIÓN INCORRECTA", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                    }
                    else
                    {
                        int codigo = Convert.ToInt32(dgvNotificadoresModificar.CurrentRow.Cells["Código Notificador"].Value.ToString());
                        string nombre = dgvNotificadoresModificar.CurrentRow.Cells["Nombre Notificador"].Value.ToString();
                        if (System.Windows.Forms.Application.OpenForms["frmCambiarNombre"] is frmCambiarNombre cn)
                        {
                            cn.Focus();
                        }
                        else
                        {
                            frmCambiarNombre cn2 = new frmCambiarNombre(codigo, nombre);
                            cn2.Show();
                        }
                    }
                }
            ...
            {
                MessageBox.Show("Error al modificar el notificador seleccionado. Detalles: " + ex.Message, "ERROR", MessageBoxButtons.OK,
        ...
        private void BtnBuscarFUN_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    System.Data.DataTable consulta = new ArchivoDAL().BuscarPorFUN(txtFUN.Text);
        ...
        private void TcPanelPrincipal_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    txtFUN.Focus();
                    frmMantenedorPrincipal mp = System.Windows.Forms.Application.OpenForms["frmMantenedorPrincipal"] as frmMantenedorPrincipal;
        ...
        private void frmMantenedorPrincipal_Load(object sender, EventArgs e)
        {
            //notificador
            try
            {
        ...
        private void BtnAsignar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                int actualizar = new ArchivoDAL().ActualizarNotificador(codigo_notificador, fecha_carga, txtFUN.Text);
        ...
        private void PbInformacionElimina_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Para poder eliminar, debe dar click derecho del Mouse sobre la columna 'Código Notificador'. En la opción desplegable dar click en 'Eliminar Notificador'.",
                "INFORMACIÓN PARA ELIMINAR", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }

        private void PnInformacionModifica_Click(object sender, EventArgs e)
        {
            MessageBox.Show("Para poder modificar, debe dar click derecho del Mouse sobre la columna 'Código Notificador'. En la opción desplegable dar click en 'Modificar Información'.",
                "INFORMACIÓN PARA MODIFICAR", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
        }
        ...
        private void BtnBuscarDescargar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    System.Data.DataTable consulta = new ArchivoDAL().BuscarFUNNotificador(txtFunDescargar.Text);
                    if (consulta.Rows.Count == 0)
                    {
                    ...
                    else
                    {
                        foreach (System.Data.DataRow item in consulta.Rows)
                        {
                            txtFunDescargar.Text = item["sa_archivo_fun"].ToString();
                            txtFunDescargar.ReadOnly = true;
                            txtTipoFunDescargar.Text = item["sa_tipo_fun"].ToString();
                            txtNombreEmpresaDescargar.Text = item["sa_archivo_nombre_empresa"].ToString();
                            txtComunaDescargar.Text = item["sa_archivo_comuna_empresa"].ToString();
                            txtNotificadorDescargar.Text = item["sa_notificador_nombre"].ToString();
                            txtDireccionDGDescargar.Text = item["sa_archivo_direccion_dg"].ToString();
                            txtFechaIngresoDescargar.Text = item["sa_archivo_fecha_ingreso"].ToString();
                            txtFechaCargaDescargar.Text = item["sa_archivo_fecha_carga"].ToString();
                            txtFechaNotificacionDescargar.Text = item["sa_archivo_fecha_notificacion"].ToString();
                            txtFechaDGDescargar.Text = item["sa_archivo_fecha_dg"].ToString();
                            txtRutAfiliadoDescargar.Text = item["sa_archivo_rut_afiliado"].ToString();
                            txtDireccionEmpresaDescargar.Text = item["sa_archivo_direccion_empresa"].ToString();
                            txtFechaProcesoDescargar.Text = item["sa_archivo_fecha_proceso"].ToString();
                            txtFechaRechazoDescargar.Text = item["sa_archivo_fecha_rechazo"].ToString();
                            txtFechaFiniquitoDescargar.Text = item["sa_archivo_fecha_finiquito"].ToString();
                            ...
                            estado_carga = true;
                            //cargar motivos
                            cmbMotivoDescargar.DataSource = new MotivoDAL().CargarListaSeleccionado(txtFunDescargar.Text);
                            cmbMotivoDescargar.DisplayMember = "sa_motivo_desc";
                            cmbMotivoDescargar.ValueMember = "sa_motivo";
                            //cargar estados de error
                            cmbObervacionDescargar.DataSource = new EstadoRechazadoDAL().CargarListaSeleccionado(txtFunDescargar.Text);
        ...
        private void BtnDescargar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (cmbMotivoDescargar.Text == "Habilitado")
                {
                    ...
                    int actualizar = new ArchivoDAL().ActualizarConHabilitado(txtNombreEmpresaDescargar.Text, txtComunaDescargar.Text, txtDireccionEmpresaDescargar.Text,
                ...
                else if (cmbMotivoDescargar.Text == "Rechazado")
                {
                    ...
                    int actualizar = new ArchivoDAL().ActualizarSinHabilitado(txtNombreEmpresaDescargar.Text, txtComunaDescargar.Text, txtDireccionEmpresaDescargar.Text,
        ...
        private void BtnVerFunesPendientes_Click(object sender, EventArgs e)
        {
            ...
            {
                dgvFunesPendientes.DataSource = null;
                System.Data.DataTable consulta_funes = new ArchivoDAL().VerFunesPendientes();
        ...
        private void BtnFunPendienteBuscar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                System.Data.DataTable consulta_funes = new ArchivoDAL().VerFunesPendientesNotificador(notificador);
                dgvFunesPendientes.DataSource = consulta_funes;
                Formato_dgv_funes_pendientes();
            }
            catch (Exception ex)
        ...
        private void BtnGenerarReporteFuenasPendientes_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    ExportarExcel(dgvFunesPendientes, "funesPendientes.xlsx");
        ...
        private void BtnBuscarDescargaAutomatica_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    System.Data.DataTable consulta = new ArchivoDAL().BuscarDatosFun(txtFunDescargaAutomatica.Text);
                    if (consulta.Rows.Count == 0)
                    {
                    ...
                    else
                    {
                        ...
                        foreach (System.Data.DataRow item in consulta.Rows)
                        {
                            if (string.IsNullOrWhiteSpace(item["sa_archivo_firma"].ToString()))
                            {
                                firma = "0";
                            }
                            else
                            ...
                            if (string.IsNullOrWhiteSpace(item["sa_archivo_timbre"].ToString()))
                            {
                                timbre = "0";
                            }
                            else
                            ...
                            dgvDescargaAutomatica.Rows.Add(txtFunDescargaAutomatica.Text, item["sa_archivo_nombre_empresa"].ToString(), item["sa_archivo_direccion_empresa"].ToString(),
                                item["sa_archivo_comuna_empresa"].ToString(), item["sa_archivo_rut_afiliado"].ToString(), "Habilitado",
        ...
        private void BtnFinalizarDescargaAutomatica_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    string orden = "";
                    int actualizar = 0;
                    bool firma = false, timbre = false, estado_firma = true, estado_timbre = true, estado_fecha = true;
                    ...
                    if (estado_firma == true && estado_timbre == true && estado_fecha == true)
                    {
                        foreach (DataGridViewRow row in dgvDescargaAutomatica.Rows)
                        {
                            ...
                            string fecha_rendicion = row.Cells["Fecha Rendición"].Value.ToString();//por defecto
                            ...
                            else if (row.Cells["Firma"].Value.ToString() == "0" || string.IsNullOrWhiteSpace(row.Cells["Firma"].Value.ToString())
                                || row.Cells["Firma"].Value.ToString() == "")
                            {
                                firma = false;
                            }
                            ...
                            else if (row.Cells["Timbre"].Value.ToString() == "0" || string.IsNullOrWhiteSpace(row.Cells["Timbre"].Value.ToString())
                                || row.Cells["Timbre"].Value.ToString() == "")
                            {
                                timbre = false;
                            }
                            string fecha_descarga = DateTime.Now.Date.ToShortDateString().Replace("/", "-");
                            ...
                            actualizar = new ArchivoDAL().DescargaHabilitados(fecha_rendicion, fecha_notificacion, fecha_descarga, habilitado, estado_rechazado, firma, timbre, Sesion.IdUsuario, orden, "HAB", fun);
                        }
                    }
                    if (actualizar >= 1)
                    {
                        ExportarExcel(dgvDescargaAutomatica, "DescargaHabilitadosConsalud.xlsx");
        ...
        private void BtnAgregarCargaAutomatica_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        System.Data.DataTable consulta = new ArchivoDAL().VerParaCargaAutomatica(txtFunCargaAutomatica.Text);
                        if (consulta.Rows.Count == 0)
                        {
                        ...
                        else
                        {
                            ...
                            foreach (System.Data.DataRow item in consulta.Rows)
                            {
                                ...
                                rut_afiliado = item["sa_archivo_rut_afiliado"].ToString();
                                System.Data.DataTable anterior = new ArchivoDAL().VerAnterior(rut_afiliado, txtFunCargaAutomatica.Text);
                                foreach (System.Data.DataRow dr in anterior.Rows)
                                {
                                    direccion_dg = dr["sa_archivo_direccion_dg"].ToString();
                                }
                                ...
                                System.Data.DataTable inubicable = new ArchivoDAL().VerInubicable(rut_afiliado);
                                ...
                                {
                                    dgvCargaAutomatica.Rows.Add(txtFunCargaAutomatica.Text, item["sa_tipo_fun"].ToString(), cmbNotificadorCargaAutomatica.Text,
                                        item["sa_archivo_rut_afiliado"].ToString(), item["sa_archivo_nombre_empresa"].ToString(),
                                        item["sa_archivo_direccion_empresa"].ToString(), item["sa_archivo_comuna_empresa"].ToString(), item["sa_motivo_desc"].ToString(),
                                        item["sa_estados_desc"].ToString(), direccion_dg, item["sa_archivo_fecha_ingreso"].ToString(),
                                        item["sa_archivo_fecha_carga"].ToString(), item["sa_archivo_fecha_notificacion"].ToString(), item["sa_archivo_fecha_dg"].ToString(),
                                        item["sa_archivo_fecha_proceso"].ToString(), item["sa_archivo_fecha_rendicion"].ToString(), item["sa_archivo_fecha_rechazo"].ToString(),
        ...
        {
            dgvCargaAutomatica.ColumnCount = 20;
            ...
            dgvCargaAutomatica.Columns[18].Name = "Firma";
            dgvCargaAutomatica.Columns[19].Name = "Timbre";
            //dgvCargaAutomatica.Rows.Clear();
        ...
        private void BtnIngresarCargaAutomatica_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        string fun = row.Cells["FUN"].Value.ToString();
                        actualizar = new ArchivoDAL().ActualizarNotificador(codigo_notificador, fecha_carga, fun);
                    }
                    if (actualizar >= 1)
                    {
        ...
        private void BtnVerificarDescargarFueraPlazo_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    System.Data.DataTable consulta = new ArchivoDAL().VerParaDescargaFueraPlazo(txtFunDescargarFueraPlazo.Text);
                    if (consulta.Rows.Count == 0)
                    {
                        MessageBox.Show("No se han encontrado registros del FUN ingresado.", "SIN DATOS", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                        txtFunDescargarFueraPlazo.Text = "";
                    ...
                    else
                    {
                        ...
                        foreach (System.Data.DataRow item in consulta.Rows)
                        {
                            ...
                            dgvDescargarFueraPlazo.Rows.Add(txtFunDescargarFueraPlazo.Text, item["sa_archivo_nombre_empresa"].ToString(), item["sa_archivo_direccion_empresa"].ToString(),
                                item["sa_archivo_comuna_empresa"].ToString(), item["sa_archivo_rut_afiliado"].ToString(), "Habilitado F.P.",
                                Convert.ToDateTime(fecha_rendicion).ToShortDateString().Replace("/", "-"), "", firma, timbre);
                ...
                else
                {
                    ...
                    if (estado_fecha == true && estado_firma == true && estado_timbre == true)
                    {
                        ...
                        foreach (DataGridViewRow row in dgvDescargarFueraPlazo.Rows)
                        {
                            ...
                            orden = DateTime.Now.TimeOfDay.ToString();
                            System.Threading.Thread.Sleep(10);
                            actualizar = new ArchivoDAL().ActualizarFueraPlazo(fecha_rendicion, fecha_notificacion, fecha_descarga, motivo, firma, timbre, Sesion.IdUsuario, orden, "FPL", fun);
                        }
                        ExportarExcel(dgvDescargarFueraPlazo, "descargarFueraPlazoConsalud.xlsx");
        ...
        private void BtnVerificarDescargarRechazados_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string fecha_rendicion = DateTime.Now.Date.ToShortDateString().Replace("/", "-");
                    System.Data.DataTable consulta = new ArchivoDAL().BuscarDatosFun(txtFunDescargarRechazados.Text);
                    foreach (System.Data.DataRow item in consulta.Rows)
                    {
                        string firma = "", timbre = "";
                        if (string.IsNullOrWhiteSpace(item["sa_archivo_firma"].ToString()))
                        ...
                        dgvDescargarRechazados.Rows.Add(txtFunDescargarRechazados.Text, item["sa_archivo_nombre_empresa"].ToString(), item["sa_archivo_direccion_empresa"].ToString(),
                                item["sa_archivo_comuna_empresa"].ToString(), item["sa_archivo_rut_afiliado"].ToString(), cmbDescargarRechazados.Text,
        ...
        private void BtnFinalziarDescargarRechazados_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        //campo fecha notificacion vacio
                        if (row.Cells[7].Value == null || row.Cells[7].Value.ToString() == "")
                        {
                            MessageBox.Show("Hay un campo de fecha que está sin completar. El error está en la fila que se ha marcado.", "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    ...
                    if (estado_fecha == true && estado_timbre == true && estado_firma == true && estado_fecha_rechazo == true)
                    {
                        ...
                        foreach (DataGridViewRow row in dgvDescargarRechazados.Rows)
                        {
                            ...
                            actualizar = new ArchivoDAL().DescargarRechazado(fecha_rendicion, fecha_notificacion, fecha_descarga, fecha_finiquito, fecha_rechazo, rechazado, codigo_rechazo, firma, timbre, Sesion.IdUsuario, orden, "REC", fun);
                            if (codigo_rechazo == 7)
                            {
                                if (!Sesion.VerificarInubicable(rut))
                                {
                                    //NO ESTÁ EN INUBICABLE
                                    int ingresa = new ArchivoDAL().IngresaInubicable(rut);
                                }
                            }
                        }
                        if (actualizar >= 1)
                        {
                            ExportarExcel(dgvDescargarRechazados, "DescargaRechazadosConsalud.xlsx");
                            MessageBox.Show("Actualización correcta.", "DATOS ACTUALIZADOS", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);
                            txtFunDescargarRechazados.Text = "";
                            txtFunDescargarRechazados.Focus();
                            dgvDescargarRechazados.Rows.Clear();
